...
public class Table {
	...
	public void mises(boolean isPreflop){
		...
		while(!miseOk){
			...
			while(!saisieCorrecte(saisie, j)){
				if(!j.isFolded()){
					System.out.println("\nAu tour de " + j.getName() + " de miser, la mise actuelle est de " + currentMise + ". " + j.getName() + " a déjà misé " + j.getMise());
			...
			if(nbrJoueursQuiOntMise < joueurs.size() && !isPreflop) //on s'assure que tous les joueurs ont bien eu le temps de miser une fois
				miseOk = false;
	 ...
	 */
	public boolean saisieCorrecte(String saisie, Joueur j){
